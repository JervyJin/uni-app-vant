<template>
	<view class="banner">
		...
		<form class="input-box" action="javascript:return true;">
			...
			<input class="uni-input" placeholder="河南省机关事务管理局" @click="goHotSearch" confirm-type="search" @confirm="searchResult"
			 style="background: #F2F2F2; padding: 17rpx 78rpx;margin-top: 21rpx;border-radius: 28rpx;" />
		</form>
		<!-- <HMsearch></HMsearch> -->
		...
		<view class="tips">
			<view class="tips_content">
				...
				<view class="tips_content_display">
					...
					<textarea v-model="content" placeholder-style="color:#B5B5B5" placeholder="请输入问题标题（20字以内）" style="background:rgba(249,249,249,1);border-radius:8rpx;height: 265rpx;line-height:42rpx;padding: 30rpx 20rpx;display: table-cell" />
					...
					<view style="padding: 30rpx;background:rgba(249,249,249,1);border-radius:8rpx;display: flex;align-items: center;">
						<view v-for="(a,i) in temp_img" :key="i" style="position: relative;">
							...
							<view style="position: absolute;width: 30upx;height: 30upx;top:20%;right: 20%;" @click="del_img(i)">
								<image src="../../static/close1.png" style="width: 30upx;height: 30upx" mode="" ></image>
							</view>
						</view>
						
						<view class="" style="width: 138rpx; height: 138rpx;border:1px solid rgba(217,217,217,1);border-radius:8rpx;text-align: center;color: #B5B5B5;" @click="choose_img">
					...
					<radio-group @change="radioChange" class="flex_row_left">
						<label class="uni-list-cell uni-list-cell-pd flex_row_left" style="margin: 0 20upx;" v-for="item in shenfen_arr"
						 :key="item.value">
							<view>
								<radio :value="(item.value).toString()" :checked="item.value == current_shenfen" />
							</view>
							<view>{{item.name}}</view>
						</label>
					</radio-group>
	...
	export default {
		...
		methods: {
			radioChange(evt) {
				...
				this.current_shenfen = evt.detail.value;
			
				for (let i = 0; i < this.shenfen_arr.length; i++) {
					this.shenfen_arr[i].checked = 'false';
					if (this.shenfen_arr[i].value === evt.target.value) {
						this.current_shenfen = i;
						this.shenfen_arr[i].checked = 'true';
						break;
					}
				}
			...
			submit(){
				...
				this.req.request('/addwt/'+openid, {
					...
					sfnmStatus:this.current_shenfen,
